...
- **Webhook hardening (don't skip this):**
  - Verify the callback is genuinely from Safaricom — check source IP against Safaricom's published ranges and/or validate any signature/shared-secret the Daraja API provides, rather than trusting any request that hits the endpoint.
  - **Idempotency:** M-Pesa (like most payment providers) may retry a callback if it doesn't get a fast enough response. Use the M-Pesa transaction reference as a unique constraint on `payments.transaction_ref` so a retried callback can't create a duplicate payment or double-mark an invoice as paid.
  - Log every callback received (payload + outcome) to a dedicated table or the Edge Function logs — if a payment silently fails to record, this is the only way to reconstruct what happened for a tenant who insists they paid.
...
### 11.8 Infrastructure & Operations

This is the layer that keeps the app running safely once real tenants and real money are moving through it — set up before production, not after something breaks.

**Environments**
- Keep separate Supabase projects for **dev / staging / production**. Never test migrations, RLS policy changes, or webhook code directly against production.
- Manage secrets (Supabase keys, M-Pesa credentials, SMS provider keys) via `.env` files locally (never committed — confirm `.gitignore` covers this) and via your hosting provider's environment-variable settings (Vercel/Netlify) or GitHub Actions secrets for CI.

**CI/CD**
- A GitHub Actions workflow that, on every pull request: runs typecheck/lint, runs the RLS/pgTAP test suite from Phase 1, and runs any Edge Function tests.
- Database migrations live as versioned files in the repo (via the Supabase CLI) and get reviewed like code — never hand-edit the schema directly in the production dashboard.
- Deploy to staging automatically on merge to `main`; promote to production as a deliberate, manual step.

**Backups & data retention**
- Confirm what backup tier your Supabase plan includes (daily backups vs. point-in-time recovery) and actually test a restore at least once before launch — an untested backup is not a backup.
- Because this system stores tenant PII and financial records, define a data retention/export process early: how would you export or delete a specific tenant's data on request, per Section 11.10's Data Protection Act note.

**Monitoring & observability**
- Use Supabase's built-in Postgres, Auth, and Edge Function logs as the first line of visibility.
- Add frontend error tracking (e.g. Sentry) so a broken dashboard screen surfaces to you, not just to a confused landlord.
- **Alert on payment webhook failures specifically** — this is the one failure mode where "nobody noticed" directly costs the client money and trust. A failed M-Pesa callback that never gets retried or flagged means a tenant paid and the system doesn't know it.

**Rate limiting & abuse protection**
- Public-facing Edge Functions (especially the M-Pesa webhook and any signup endpoint) should have basic rate limiting to prevent abuse, separate from the idempotency handling in Section 11.4.

### 11.9 Build order for the product app

1. Set up dev/staging/production Supabase projects and the CI/CD pipeline (Section 11.8) — do this first, not last.
2. Set up a Supabase project (Postgres, Auth, Edge Functions, Storage) — separate from anything used for the marketing site.
3. Define the Postgres schema (Section 11.2) and **enable + test Row-Level-Security policies first**, before any UI.
4. Build auth: signup (creates org + owner profile), login, role-based route guards.
5. Build core CRUD screens: Properties → Units → Tenants.
6. Build manual invoice creation, then automate it (Section 11.5).
7. Integrate M-Pesa in **sandbox mode** first, including the webhook hardening from Section 11.4.
8. Build payment recording + auto-reconciliation against invoices.
9. Add SMS notifications (Section 11.6).
10. Build reporting/dashboard views (Section 11.7).
11. Add maintenance request tracking.
12. Set up monitoring, alerting, and a tested backup/restore process (Section 11.8) — before any production data touches the system, not after.
13. Full RLS policy audit — specifically test that Org A can never read/write Org B's rows, across every table and every Edge Function.
14. Staging deploy → test with realistic data volume → production deploy.

### 11.10 Requires client input before this can be built (separate from the marketing-site list in Section 10)
...
This is a separate repo/project from the marketing site. Phases 6 and 7 below are **blocked** until the Section 11.10 decisions (M-Pesa credentials, SMS provider) are confirmed with the client — don't run those prompts until you have real values to fill in.

**Phase 0 — Environments + CI/CD**
```
Using docs/PLAN.md Section 11.8 as context, before any application code:
1. Set up separate dev/staging/production Supabase projects (or document the plan to do so — production can wait until closer to launch, but the structure should be decided now).
2. Set up a GitHub Actions workflow that runs typecheck, lint, and (once Phase 1 exists) the RLS test suite on every pull request.
3. Confirm .env files are gitignored and document which environment variables the app needs in a .env.example file.

This is setup only — no feature code yet.
```
...
No tenant-facing login yet — that's still undecided per Section 11.10.
```
...
**Phase 6 — M-Pesa Integration (blocked on credentials)**
```
Using docs/PLAN.md Section 11.4 as context, build the M-Pesa Daraja API integration in sandbox mode using these credentials: [insert sandbox credentials]. Build the Supabase Edge Function webhook that receives payment confirmations, verifies the callback is genuinely from Safaricom, inserts a payments row (using transaction_ref as a unique constraint so retried callbacks can't create duplicates), updates the matching invoice's status to 'paid', and logs every callback received regardless of outcome. Do not switch to production credentials until this is tested end-to-end in sandbox, including a simulated duplicate callback.
```

**Phase 7 — SMS Notifications (blocked on provider choice)**
```
...
**Phase 10 — Monitoring, Backups & Secrets**
```
Using docs/PLAN.md Section 11.8 as context, before any production deploy:
1. Wire up frontend error tracking (e.g. Sentry).
2. Set up an alert specifically for M-Pesa webhook failures — this must not fail silently.
3. Confirm the Supabase backup tier in use and document (and actually run) a test restore.
4. Confirm all secrets (Supabase keys, M-Pesa credentials, SMS provider keys) are only in environment variables/CI secrets, never committed to the repo.
```

**Phase 11 — Final Security Audit (before any production deploy)**
```
